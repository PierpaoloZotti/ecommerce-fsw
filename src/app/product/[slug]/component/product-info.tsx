...
const ProductInfo = ({
  ...
  const handleDecrement = () => {
    setBagQty((prev) => (prev === 1 ? prev : prev - 1));
  };
  ...
  return (
    <div className="flex flex-col gap-0.5 px-5">
      ...
      {discountPercentage > 0 ? (
        <div className="mt-3 flex flex-col">
          <div className="flex items-center">
            <h2 className="text-xl font-bold">R$ {totalPrice}</h2>
          ...
          <h3 className="text-sm text-primary-foreground/70">
            from: R$ <span className="line-through">{Number(basePrice)}</span>
          </h3>
        </div>
      ) : (
        <h2 className="mt-3 text-xl font-bold">R$ {totalPrice}</h2>
      )}
      ...
      <Button className="mt-7 w-full rounded-lg bg-primary text-lg font-bold">
        Add to cart
      </Button>
